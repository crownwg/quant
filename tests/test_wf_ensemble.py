...
def test_walk_forward_summary_has_five_arms():
    p = _prices()
    res = _wf(_Args(), p)
    keys = list(res["summary"]["key"])
    assert keys == ["walk_forward", "smooth", "ensemble", "full_sample_best", "default"]
    # 五条曲线都要有每折记录，长度一致
    lens = {k: len(v) for k, v in res["curves"].items()}
    assert len(set(lens.values())) == 1, lens


def test_walk_forward_folds_record_all_three_selection_modes():
    p = _prices()
    res = _wf(_Args(), p)
    f = res["folds"]
    for col in ("chosen", "smooth_chosen", "test_return", "test_return_smooth",
                "test_return_ensemble"):
        assert col in f.columns, col
    # 三种选参方式看的是同一段测试窗，收益不应完全一样（同参数时才会一样）
    assert not np.allclose(f["test_return"], f["test_return_ensemble"])
...
def test_walk_forward_ensemble_note_reports_delta():
    ...
    assert "邻域平滑" in note and "全组合集成" in note
    # 差值必须与汇总表自洽
    sm = res["summary"].set_index("key")
    d = float(sm.loc["ensemble", "total_return"]) - float(sm.loc["walk_forward", "total_return"])
    assert f"{d:+.1%}" in note


def test_walk_forward_report_renders_five_arms(tmp_path):
    """报告生成必须能吃下五条臂（旧版硬编码三条，容易漏改）。"""
    import json
    import re
    p = _prices()
...
def test_walk_forward_single_combo_still_works():
    """候选只剩 1 组时（等于没得选），三条自适应臂必须一致而不是崩掉。"""
    p = _prices()
    res = grid.walk_forward_search(
