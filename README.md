...
### 9.2 不做单点择优：邻域平滑 与 全组合集成

9.1 的结论是「argmax 挑到的大概率是噪声」。那就别挑了。两条替代方案：

| 方案 | 做法 | 对应的问题 |
|---|---|---|
| **邻域平滑选参** | 先在参数曲面上做一次均值滤波（每个点取「自己 + 相邻档」的训练期均值），再取 argmax | argmax 永远踩到孤立尖峰 |
| **全组合集成** | 干脆不选，把所有候选的权重矩阵等权平均后一起持有（一笔净额委托，成本只算一次） | 「我知道哪个参数对」这个假设本身可能是错的 |

实现细节（`quant/grid.py`）：

- `_neighbor_map`：邻域 = 各维度在候选序列上**相差不超过 1 档**，且**择时模式必须相同**
  （跨模式是「换了方法」，不是「相邻参数」）。邻域是对称的，有单测锁。
- `smooth_scores`：纯函数，只做邻域均值，便于单测。
- `ensemble_weights`：**平均权重**而不是「平均各条净值曲线」——后者要 N 份资金
  各付一遍成本，既不可实现也把成本重复计算了。

**实测（同一段测试窗，唯一差别是「怎么选参数」）**

| 折 | 测试窗 | argmax 选中 | 邻域平滑选中 | argmax | 邻域 | 集成 |
|---|---|---|---|---|---|---|
| 1 | 2022-01~2022-08 | ma(40日,2%) | 不择时 | +6.0% | −3.3% | −1.2% |
| 2 | 2022-08~2023-02 | momentum(90日,2%) | dual(200日,0%) | **−5.7%** | **+5.2%** | +3.9% |
| 3 | 2023-02~2023-08 | ma(40日,0%) | ma(40日,0%) | −2.8% | −2.8% | −4.6% |
| 4 | 2023-08~2024-03 | ma(40日,0%) | ma(40日,0%) | +0.9% | +0.9% | +0.7% |
| 5 | 2024-03~2024-09 | ma(40日,2%) | ma(40日,0%) | +1.5% | **+6.2%** | −0.3% |

**怎么诚实读这张表（容易被误读，所以拆开说）**

1. **邻域平滑赢得不平均。** 5 折里 2 折与 argmax 完全相同、1 折输 9.3pp、1 折赢 10.9pp、1 折赢 4.7pp。
   总收益 +6.4pp 里，**几乎全部来自第 2 折那一次**：argmax 在训练窗选中 `momentum(90日)`
   （跨模式、且是 momentum 组的一员），样本外直接 −5.7%；平滑因为 momentum 组整体平庸
   把它抹掉、落回 ma 附近，拿到 +5.2%。
   → **邻域平滑的真实作用是「防止一次跨模式的翻车」**，不是「每年稳定多赚」。
   不要把它当收益增强器卖；它的价值是**降低选参这一步的方差**。
2. **集成的价值在回撤，不在收益。** 全组合集成收益平庸（−1.7%，略差于 argmax），
   但**最差折回撤 −8.3% 是五条臂里最小的**（对照：argmax −15.5%、不择时 −17.9%）。
   原因很直白：不把资金押在一次参数选择上，单折踩雷的幅度自然被摊薄。
   → 如果你的目标是「别在某半年被腰斩」，集成是这五条里唯一真正负责的选择。
3. **哪怕这次的 +6.4pp 是运气，换掉 argmax 仍然是对的。**
   argmax 的执行逻辑（挑尖峰）在原理上就是错的，平滑/集成的逻辑
   （不把结论押在一次选择上）在原理上是对的。**做法正确 + 结果不保证**，
   和「做法错误 + 结果偶然很好」是完全不同的两个状态。
4. **集成的改进方向**（尚未做）：这次集成是无差别平均全部 31 组，
   把已知表现最差的 momentum 组也等权持有，被拖累了。
   更合理的做法是**只在参数曲面的「好区域」内集成**（如只在 off + ma + dual 组内平均，
   或先按邻域平滑排序取前 K 组再平均）。这是下一步。
...
```bash
.venv/Scripts/python.exe -m pytest tests -q     # 140 项
```
...
以及 P2-3 的邻域定义（不跨择时模式、关系对称）、邻域平滑（尖峰让位给高原）、
...
2. **集成的改良：只在「好区域」内集成**——现在的全组合集成是无差别平均全部候选，
   把已知最差的 momentum 组也等权持有了，被拖累（收益平庸，只换来回撤最小）。
   更合理的做法是**先按邻域平滑排序取前 K 组再平均**，或按模式的组均值加权
   （ma 权重高、momentum 权重低），把「哪个区域好」这个稳健信息用上、又不押单点。
